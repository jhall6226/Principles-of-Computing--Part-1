...
class TwentyFortyEight:
	...
	def __path_through_grid__(self, start_cell, direction, num_steps):
		"""
		...
		for step in range(length_of_line):
			...
			path.append(self.grid[row][col])
			
		return path
		
	def __set_values_on_path__(self, start_tile, direction, line):
		"""
		Function that iterates through the tiles in a grid
	...
	def move(self, direction):
		...
		"""
		starting_tiles = {  UP: [(0,col) for col in range(self.grid_width)],
		...
		lines_to_merge = []
		length_of_lines = max(abs(OFFSETS[direction][0]*self.grid_height),abs(OFFSETS[direction][1]*self.grid_width))
		
		for tile in starting_tiles[direction]:
			lines_to_merge.append(self.__path_through_grid__(tile,direction,length_of_lines))
		...
		if moved:
			for index in range(len(merged_lines)):
				self.__set_values_on_path__(starting_tiles[direction][index],direction,merged_lines[index])	
			self.new_tile()	
	...
	def new_tile(self):
		...
		empty_tile = random.randint(0,self.__num_empty_tiles__()-1)
		...
		TwoOrFour = random.Random()
		if TwoOrFour.uniform(0,1) >= 0.9:
			self.grid[empty_tile_row][empty_tile_col] = 4
		else:
			self.grid[empty_tile_row][empty_tile_col] = 2		
...
test_game = TwentyFortyEight(5,4)